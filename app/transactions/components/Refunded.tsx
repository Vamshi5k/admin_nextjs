...
const Refund = () => {
  const [refunded, setRefunded] = useState<Transaction[]>([]);
  ...
  const GetRefunded = async () => {
    try {
      ...
      if (res?.data) {
        const refunded_data = res?.data.filter((item: Transaction) => item?.status === 4);
        setRefunded(refunded_data);
      }
    } catch (error) {
      console.log(error);
      setError('Error Fetching Refunded Transactions');
      toast({
        description: 'Error Fetching Refunded Transactions',
        variant: 'destructive',
      });
    } finally {
      setLoading(false);
  ...
  const currentTransaction = refunded.slice(indexofFirstTransaction, indexofLastTransaction);



  const totalPages = Math.ceil(refunded?.length / itemsPerPage);
    ...
    <div className='mt-10'>
      <Card>
        <CardHeader>
          ...
          <CardDescription className='mt-2'>Overall {refunded?.length} Transactions</CardDescription>
        </CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
                ...
                currentTransaction?.length > 0 ? (
                  currentTransaction?.map((item: Transaction, index) => (
                    <TableRow key={index}>
                      ...
                      <TableCell>
                        <Badge variant={item?.status === 4 ? "secondary" : "default"}>
                          {item?.status === 4 ? 'Refunded / Replacement' : `Status ${item?.status}`}
                        </Badge>
                      </TableCell>
                      <TableCell>
